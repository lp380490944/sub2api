...
func TestBuildOAuthRequest_BillingMatchesWireUserAgent(t *testing.T) {
	...
	for _, endpoint := range []string{"messages", "count_tokens"} {
		for _, tc := range []struct {
			...
			// fork 语义：mimic 路径先强制 DefaultHeaders，再用账号指纹覆盖。
			// 非 claude-cli 客户端拿到的是"按账号分桶版本"的新指纹（不读缓存），
			// 出站 UA 必须是该指纹 UA 而不是 DefaultHeaders。
			{name: "mimic_uses_account_fingerprint", mimic: true, identity: true},
			...
			t.Run(endpoint+"/"+tc.name, func(t *testing.T) {
				...
				// 账号 12 的分桶落在 recent[1]，让分桶版本与 DefaultHeaders 明显不同。
				SetCachedRecentVersions([]string{"2.1.999", "2.1.900"})
				t.Cleanup(func() { SetCachedRecentVersions(nil) })
				c, _ := gin.CreateTestContext(httptest.NewRecorder())
				c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", nil)
				if !tc.mimic {
					// 真 CC 客户端：走账号级持久缓存；客户端版本低于缓存 → 保留缓存 UA。
