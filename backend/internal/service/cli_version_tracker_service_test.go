...
// cliTrackerFakeSettingRepo 内存版 SettingRepository，仅实现 tracker 用到的方法。
type cliTrackerFakeSettingRepo struct {
	store map[string]string
}

func newCLITrackerFakeSettingRepo(initial map[string]string) *cliTrackerFakeSettingRepo {
	store := map[string]string{}
	for k, v := range initial {
		store[k] = v
	}
	return &cliTrackerFakeSettingRepo{store: store}
}

func (f *cliTrackerFakeSettingRepo) Get(ctx context.Context, key string) (*Setting, error) {
	if v, ok := f.store[key]; ok {
		return &Setting{Key: key, Value: v}, nil
	}
	return nil, nil
}
func (f *cliTrackerFakeSettingRepo) GetValue(ctx context.Context, key string) (string, error) {
	return f.store[key], nil
}
func (f *cliTrackerFakeSettingRepo) Set(ctx context.Context, key, value string) error {
	f.store[key] = value
	return nil
}
func (f *cliTrackerFakeSettingRepo) GetMultiple(ctx context.Context, keys []string) (map[string]string, error) {
...
func (f *cliTrackerFakeSettingRepo) SetMultiple(ctx context.Context, settings map[string]string) error {
	for k, v := range settings {
		f.store[k] = v
	}
	return nil
}
func (f *cliTrackerFakeSettingRepo) GetAll(ctx context.Context) (map[string]string, error) {
	out := map[string]string{}
...
func (f *cliTrackerFakeSettingRepo) Delete(ctx context.Context, key string) error {
	delete(f.store, key)
	return nil
}
...
func TestRunOnce_NpmFailureNoOp(t *testing.T) {
	...
	repo := newCLITrackerFakeSettingRepo(nil)
	svc := NewCLIVersionTrackerService(repo, config.CLIVersionTrackerConfig{
		Enabled:           false,
		NpmRegistryURL:    server.URL,
		RequestTimeoutSec: 2,
