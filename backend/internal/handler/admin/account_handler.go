...
func (h *AccountHandler) Create(c *gin.Context) {
	...
	// fork(P1-1)：OAuth / SetupToken 账号创建时，从管理员浏览器 UA 记录注册设备指纹
	// （OS/Arch），之后非 Claude Code 客户端走该账号时对外 x-stainless-os 与注册设备一致，
	// 而不是回落到全局的 Linux/arm64。API Key 账号没有"注册浏览器"概念，跳过。
	// 必须放在幂等闭包之前：req 参与幂等键计算，且闭包重放时不会再执行。
