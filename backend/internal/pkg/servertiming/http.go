...
func Do(client *http.Client, req *http.Request) (*http.Response, error) {
	...
	if req == nil || !Active(req.Context()) {
		return client.Do(req) //nolint:gosec // proxying wrapper: request is caller-constructed upstream, not raw user input
	}
	startedAt := time.Now()
	response, err := client.Do(req) //nolint:gosec // proxying wrapper: request is caller-constructed upstream, not raw user input
